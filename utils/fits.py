...
def arlo(te, y, flag_water=0):
    ...
    yy = torch.zeros(sz[:-1]).to(dev)
    yx = torch.zeros(sz[:-1]).to(dev)
    beta_yx = torch.zeros(sz[:-1]).to(dev)
    beta_xx = torch.zeros(sz[:-1]).to(dev)
    ...
    if flag_water:
        A = torch.exp(-r2[..., None] * torch.tensor(np.array(te)[None, None, None, :]).to(dev))
...
def arlo_numpy(te, y):
    """
    Calculate the R2 map using the ARLO method.

    Parameters:
    te : array_like
        Array containing TE values (in seconds).
    y : array_like
        A multi-echo data set of arbitrary dimension, with echo as the last dimension.

    Returns:
    r2 : ndarray
        R2 map (in Hz). Returns an empty array if only one echo is provided.
    """
    ...
    for j in range(nte - 2):
        alpha = (te[j + 2] - te[j]) * (te[j + 2] - te[j]) / 2 / (te[j + 1] - te[j])
        tmp = (2 * te[j + 2] ** 2 - te[j] * te[j + 2] - te[j] ** 2 + 3 * te[j] * te[j + 1] - 3 * te[j + 1] * te[j + 2]) / 6
        beta = tmp / (te[j + 2] - te[j + 1])
        gamma = tmp / (te[j + 1] - te[j])
...
def Fit_ppm_complex(M, opts=None):
    # Default options
    defopts = {'reltol': 1e-4, 'max_iter': 30}
    
    # Update options with any user-specified values
    if opts is None:
        opts = defopts
    else:
        for key in defopts:
            opts.setdefault(key, defopts[key])

    # Check memory usage approximation by array size
    bytes = M.nbytes
    if bytes > 1e9:
        sz = M.shape
        p1 = np.zeros(sz[0:3], dtype=M.dtype)
        dp1 = np.zeros_like(p1)
        relres = np.zeros_like(p1)
        p0 = np.zeros_like(p1)
        n = np.ceil(bytes / 1e9)
        ns = np.floor(sz[2] / n)
        for slice_start in range(0, sz[2], ns):
            rng = slice(slice_start, min(slice_start + ns, sz[2]))
            # Recursive call for slices
            p1[:, :, rng], dp1[:, :, rng], relres[:, :, rng], p0[:, :, rng] = Fit_ppm_complex(M[:, :, rng, :, :], opts)
        return p1, dp1, relres, p0

    # Modification for datasets with one echo
    if M.shape[3] == 1:
        M = np.concatenate((np.abs(M), M), axis=3)

    # Combine multiple coils together if present
    if M.shape[4] > 1:
        M = np.sum(M * np.conj(M[:, :, :, 0:1, :]), axis=4)
        M = np.sqrt(np.abs(M)) * np.exp(1j * np.angle(M))
    ...
    Y = np.angle(M[:, 0:min(3, nechos)])
    c = (Y[:, 1] - Y[:, 0])
    m = np.min(np.abs([c - 2 * np.pi, c, c + 2 * np.pi]), axis=0)
    c[m == np.abs(c - 2 * np.pi)] -= 2 * np.pi
    c[m == np.abs(c + 2 * np.pi)] += 2 * np.pi

    for n in range(min(2, nechos - 1)):
        cd = (Y[:, n + 1] - Y[:, n]) - c
        Y[cd < -np.pi, (n + 1):] += 2 * np.pi
        Y[cd > np.pi, (n + 1):] -= 2 * np.pi
    
    A = np.array([[1, 0], [1, 1], [1, 2]], dtype=M.dtype)[0:min(3, nechos), :]
    ip = np.linalg.lstsq(A, Y[:, 0:min(3, nechos)].T, rcond=None)[0]
    p0 = ip[0, :].T
    p1 = ip[1, :].T

    dp1 = p1.copy()
    tol = np.linalg.norm(p1) * opts['reltol']
    iter = 0

    while np.linalg.norm(dp1) > tol and iter < opts['max_iter']:
        iter += 1
        v1 = np.ones(nechos, dtype=M.dtype)
        v2 = np.arange(nechos, dtype=M.dtype)
        W = np.abs(M) * np.exp(1j * (np.outer(p0, v1) + np.outer(p1, v2)))

        pr1 = np.sum(np.conj(1j * W) * v1 * (M - W), axis=1)
        pr2 = np.sum(np.conj(1j * W) * v2 * (M - W), axis=1)

        a11 = np.sum(np.abs(M) ** 2 * v1 ** 2, axis=1)
        a12 = np.sum(np.abs(M) ** 2 * v1 * v2, axis=1)
        a22 = np.sum(np.abs(M) ** 2 * v2 ** 2, axis=1)
        d = a11 * a22 - a12 ** 2
        ai11 = a22 / d
        ai12 = -a12 / d
        ai22 = a11 / d

        dp0 = np.real(ai11 * pr1 + ai12 * pr2)
        dp1 = np.real(ai12 * pr1 + ai22 * pr2)
        dp1[np.isnan(dp1)] = 0
        dp0[np.isnan(dp0)] = 0
    ...
    dp1 = np.sqrt(np.real(ai22))
    dp1[np.isnan(dp1)] = 0
    dp1[np.isinf(dp1)] = 0

    res = M - np.abs(M) * np.exp(1j * (np.outer(p0, v1) + np.outer(p1, v2)))
    relres = np.sum(np.abs(res) ** 2, axis=1) / np.sum(np.abs(M) ** 2, axis=1)
    relres[np.isnan(relres)] = 0

    p1[p1 > np.pi] = np.mod(p1[p1 > np.pi] + np.pi, 2 * np.pi) - np.pi
    p1[p1 < -np.pi] = np.mod(p1[p1 < -np.pi] + np.pi, 2 * np.pi) - np.pi

    p0 = p0.reshape(s0[:-1])
    p1 = p1.reshape(s0[:-1])
    dp1 = np.real(dp1).reshape(s0[:-1])
    relres = relres.reshape(s0[:-1])
