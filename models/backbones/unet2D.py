...
class convNormAct(nn.Module):
    ...
    def __init__(self, in_cs, out_cs, norm_type, kernel_size, stride, padding, is_act = True):
        ...
        norm_2d, norm_params = getNormModuleND(dimension = 2, norm_type = norm_type)

        self.conv =  nn.Conv2d(in_cs, out_cs, kernel_size, stride, padding, bias = False)
        if norm_type is not None:
            self.norm = norm_2d(*(norm_params + [out_cs]))
        else:
    ...
    def __init__(self, in_cs, out_cs, norm_type, downsample):
        ...
        if in_cs != out_cs or stride != 1:
            ...
            self.shortcut = nn.Sequential(
                nn.Conv2d(in_cs, out_cs, 1, stride, 0, bias = False),
                norm_2d(*(norm_params + [out_cs])),
            )
        else:
            self.shortcut = nn.Sequential() 
...
class unetDownSample(nn.Module):
    ...
    def __init__(self, channels, down_type = 'conv', norm_type = 'GN_8'):
        ...
        if down_type == 'conv':
            self.down = nn.Conv2d(channels, channels, 3, 2, 1, bias = False)
        if down_type == 'maxpool':
            self.down = nn.MaxPool2d(kernel_size = 2, padding = 1)
    ...
    def __init__(self, in_cs, out_cs, is_deconv, upsample_type):
        ...
        if is_deconv:
            self.up = nn.Sequential(
                ...
                nn.ConvTranspose2d(in_cs, out_cs, kernel_size=2, stride=2, padding=0, bias=False)            
            )
        else:
